param(
    ...
    [string]$Repository = 'MHJoy99/Rainmeter-Todo-Skin',
...
function Install-Package {
    ...
    if (Test-Path -LiteralPath $bootstrapPath) {
        ...
        try {
            ...
            if ((Split-Path -Leaf $bootstrapDownload) -like 'Todo-Skin-*.zip') {
    ...
    foreach ($skin in @('Todo', 'Calendar')) {
        ...
        $preserved = @{}
        foreach ($name in @('tasks.json','ui-scale.txt','calendar-cache.json','calendar-state.json','caldav.secret','translation.secret','paper-sync.secret','gtasks-client.json','gtasks.secret')) {
...
function Check-And-Install {
    ...
    # canonical package.
    $assetName = "Todo-Skin-v$latestVersion.zip"
    ...
    try {
        ...
        if ((Split-Path -Leaf $zipPath) -like 'Todo-Skin-*.zip') {
