...
if ([string]::IsNullOrWhiteSpace($CalendarSource)) {
    $CalendarSource = Join-Path (Split-Path $projectRoot -Parent) 'Calendar'
}
...
"@
    ...
    $rmskin = Join-Path $OutputRoot ("Todo-Skin-v$Version.rmskin")
    $rmskinZip = Join-Path $OutputRoot ("Todo-Skin-v$Version.rmskin.zip")
...
function New-Package {
    ...
    $packageName = "Todo-Skin-v$Version"
    ...
    $stagingSkins = Join-Path $OutputRoot 'staging\Skins'
    $stagingTodo = Join-Path $stagingSkins 'Todo'
    $stagingCalendar = Join-Path $stagingSkins 'Calendar'
    Copy-Tree $projectRoot $stagingTodo
    if (-not (Test-Path -LiteralPath $CalendarSource)) { throw "Calendar skin source not found: $CalendarSource" }
    Copy-Tree $CalendarSource $stagingCalendar
    Copy-Tree $stagingTodo $todoRoot
    Copy-Tree $stagingCalendar $calendarRoot
    ...
    $deployDoc = Join-Path $projectRoot 'docs\RELEASE-DEPLOY.md'
    if (Test-Path -LiteralPath $deployDoc) {
        Copy-Item -LiteralPath $deployDoc -Destination (Join-Path $packageRoot 'DEPLOY.md') -Force
    }
...
function New-LegacyBootstrapPackages {
    ...
    $bootstrap = [ordered]@{
        repository = 'MHJoy99/Rainmeter-Todo-Skin'
        tag = "v$Version"
        version = $Version
        asset = "Todo-Skin-v$Version.zip"
    } | ConvertTo-Json
    ...
    foreach ($legacyFlavor in @('full','lite')) {
        $target = Join-Path $OutputRoot "Todo-Skin-v$Version-$legacyFlavor.zip"
