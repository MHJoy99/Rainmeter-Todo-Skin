...
$rainmeterRoot = 'C:\Program Files\Rainmeter'
...
$preserved = @{}
foreach ($name in @('tasks.json','ui-scale.txt','caldav.secret','translation.secret','paper-sync.secret','gtasks-client.json','gtasks.secret')) {
...
$documentsTarget = 'C:\Users\Administrator\Documents\Rainmeter\Skins\Todo'
New-Item -ItemType Directory -Path $documentsTarget -Force | Out-Null
Copy-Item -Path (Join-Path $target '*') -Destination $documentsTarget -Recurse -Force
Write-Host "Deployed Todo skin to $target and $documentsTarget"
