...
internal static class TodoLayoutProbe
{
    ...
    private static void ProbeEditor()
    {
        ...
        timer.Tick += delegate {
            Form form = Application.OpenForms.Cast<Form>().FirstOrDefault(candidate => candidate.Text == "New task");
            if (form == null) return;
            try
            {
                DpiLayoutAssertions.AssertManualScaling(form);
                DpiLayoutAssertions.AssertPixelFonts(form);
                AssertTextFits(form, "New task", true, "Todo editor title");
                AssertTextFits(form, "Notes", true, "Todo note label");
                Button expand = Descendants(form).OfType<Button>().First(button => button.Text == "Expand" || button.Text == "Collapse");
                Label note = Descendants(form).OfType<Label>().First(label => label.Text == "Notes");
                ...
                if (stage == 0)
                {
                    ...
                    if (preferred.Width > expand.ClientSize.Width + 2 || preferred.Height > expand.ClientSize.Height + 2) throw new Exception("Expand button text clipped: measured=" + preferred + ", client=" + expand.ClientSize + ", font=" + expand.Font.Size + expand.Font.Unit);
                    expand.PerformClick();
                    stage = 1;
                    return;
                }
                if (stage == 1)
                {
                    if (expand.Text != "Collapse") throw new Exception("Expand action did not open selector");
                    expand.PerformClick();
                    stage = 2;
                    return;
                }
    ...
    private static void ProbeManager()
    {
        ...
        tasks.Add(new Dictionary<string, object> {
            {"id", "probe-task"}, {"title", "Layout probe task"}, {"target", ""}, {"note", ""},
            {"labels", new List<object> { "Feature" }}, {"completed", false}, {"source", "manual"},
        ...
        Timer timer = new Timer { Interval = 80 };
        timer.Tick += delegate {
            Form form = Application.OpenForms.Cast<Form>().FirstOrDefault(candidate => candidate.Text == "All tasks");
            if (form == null) return;
            try
            {
                DpiLayoutAssertions.AssertManualScaling(form);
                DpiLayoutAssertions.AssertPixelFonts(form);
                AssertTextFits(form, "All tasks", true, "Todo manager title");
                AssertTextFits(form, "Open only", true, "Todo open-only filter");
                ...
                if (stage == 0)
                {
                    Button all = Descendants(form).OfType<Button>().First(button => button.Text.StartsWith("All", StringComparison.Ordinal));
                    all.PerformClick();
                    stage = 1;
                    return;
                }
