...
internal static class CalendarLayoutProbe
{
    ...
    private static void AssertManagerHighDpi(Form form, Button allTime, List<Control> days)
    {
        ...
        AssertTextFits(form, "Manage Events", true, "Manage Events title");
        AssertTextFits(form, "View, edit and sync your local and CalDAV calendars.", true, "Manage Events subtitle");
        AssertTextFits(form, "Filter calendars", true, "Calendar filter title");
        AssertTextFits(form, "Today", true, "Calendar today button");
        AssertTextFits(form, "Next 7 days", true, "Calendar week tab");
        AssertTextFits(form, "All time", true, "Calendar all-time tab");
        AssertTextFits(form, "Sync", true, "Event Settings button");
        AssertTextFits(form, "Sync", true, "Calendar sync button");
        AssertTextFits(form, "New event", true, "Calendar add button");
        foreach (Control month in Descendants(form).Where(control => control is Label && control.Text.Length == 7 && control.Text[4] == '-'))
            DpiLayoutAssertions.AssertFitsAt200Percent(month, true, "Calendar month title");
        foreach (Control source in Descendants(form).Where(control => control is Button && (control.Text.Contains("Local") || control.Text.Contains("CalDAV"))))
    ...
    private static Dictionary<string, object> Cache()
    {
        ...
        cache["events"] = new List<object> {
            new Dictionary<string, object> {
                ...
                {"title", "Layout probe event"}, {"start_at", start.ToString("o")}, {"end_at", start.AddHours(1).ToString("o")},
                {"all_day", false}, {"url", ""}, {"location", "Conference room"}, {"description", ""},
    ...
    private static void ProbeManager()
    {
        ...
        timer.Tick += delegate {
            Form form = Application.OpenForms.Cast<Form>().FirstOrDefault(candidate => candidate.Text == "Manage Events");
            if (form == null) return;
            try
            {
                Button allTime = Descendants(form).OfType<Button>().First(button => button.Text == "All time");
                if (stage == 0)
                {
                ...
                if (stage == 1)
                {
                    ...
                    Button today = Descendants(form).OfType<Button>().First(button => button.Text == "Today" && button.Parent == allTime.Parent);
                    today.PerformClick();
                    stage = 2;
                    return;
                }
    ...
    private static void ProbeSettings()
    {
        ...
        Timer timer = new Timer { Interval = 100 };
        timer.Tick += delegate {
            Form form = Application.OpenForms.Cast<Form>().FirstOrDefault(candidate => candidate.Text == "Event Settings");
            if (form == null) return;
            try
            {
                DpiLayoutAssertions.AssertManualScaling(form);
                DpiLayoutAssertions.AssertPixelFonts(form);
                AssertTextFits(form, "Event Settings", true, "Event Settings title");
        ...
        timer.Start();
        object[] parameters = { state, cache, false };
        try { Method("Sync").Invoke(null, parameters); }
        catch (TargetInvocationException ex) { failure = ex.InnerException ?? ex; }
        timer.Dispose();
    }
}
