...
namespace RainmeterBackend
{
    internal static class UiScale
    {
        ...
        private const float MinimumScale = 0.50F;
        private const float MaximumScale = 2.50F;
        ...
        public static void SaveMode(string mode)
        {
            ...
            if (normalized != AutoMode)
            {
                ...
                if (!Single.TryParse(normalized, NumberStyles.Float, CultureInfo.InvariantCulture, out parsed))
                    throw new ArgumentException("Invalid UI scale ratio");
    ...
    internal static class RuntimeUtil
    {
        ...
        public static readonly UnicodeEncoding Utf16 = new UnicodeEncoding(false, true);
        ...
        public static bool WriteUtf8IfChanged(string path, string text)
        {
            // Rainmeter requires UTF-16 LE with BOM for reliable Unicode rendering.
            byte[] encoded = Utf16.GetBytes(text);
            byte[] bom = Utf16.GetPreamble();
            byte[] bytes = new byte[bom.Length + encoded.Length];
            Buffer.BlockCopy(bom, 0, bytes, 0, bom.Length);
            Buffer.BlockCopy(encoded, 0, bytes, bom.Length, encoded.Length);
    ...
    internal static class LightUi
    {
        ...
        private static string HeadingGlyph(string title, string subtitle)
        {
            ...
            if (text.Contains("Schedule") || text.Contains("Time")) return "";
            if (text.Contains("All") || text.Contains("Manage") || text.Contains("Rule")) return "";
            if (text.Contains("Delete") || text.Contains("Open")) return "";
            if (text.Contains("Todo") || text.Contains("Task")) return "✓";
            return "";
        }
        ...
        {
            Form form = Form(title, 480, 250); Heading(form, title, "This action cannot be undone.");
            ...
            Button cancel = Button("Cancel", 278, 184, 84, DialogResult.Cancel), confirm = DangerButton("Delete", 372, 184, 82, DialogResult.Yes);
        ...
        {
            Form form = Form("Done", 480, 240); Heading(form, "Done", "Operation completed successfully.");
            ...
            Round(message, 10); form.Controls.Add(message);
            Button close = PrimaryButton("OK", 370, 174, 84, DialogResult.OK); form.Controls.Add(close); form.AcceptButton = close; form.CancelButton = close;
            form.ShowDialog();
        }

        public static void Error(string text)
        {
            Form form = Form("Operation incomplete", 480, 240); Heading(form, "Operation incomplete", "Please check your input and try again.");
            Label message = new Label { Text = text, Left = 26, Top = 98, Width = 428, Height = 52, ForeColor = Text, BackColor = Surface, Padding = new Padding(14, 13, 14, 8), Font = new Font("Microsoft YaHei UI", 10F) };
            Round(message, 10); form.Controls.Add(message);
            Button close = PrimaryButton("OK", 370, 174, 84, DialogResult.OK); form.Controls.Add(close); form.AcceptButton = close; form.CancelButton = close;
            form.ShowDialog();
