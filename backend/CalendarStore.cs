...
{
    private static Dictionary<string,object> NewCache(){return new Dictionary<string,object>{{"version",1},{"fetched_at",""},{"calendar_url",""},{"status",File.Exists(SecretPath)?"Not synced yet":"CalDAV not connected"},{"events",new List<object>()}};}
    ...
    private static void Shape(Dictionary<string,object> cache,Dictionary<string,object> state){if(JsonUtil.Get(cache,"events")==null)cache["events"]=new List<object>();if(JsonUtil.Get(cache,"status")==null)cache["status"]=File.Exists(SecretPath)?"Ready":"CalDAV not connected";if(JsonUtil.Get(state,"series_rules")==null)state["series_rules"]=new List<object>();if(JsonUtil.Get(state,"conversions")==null)state["conversions"]=new List<object>();if(JsonUtil.Get(state,"local_events")==null)state["local_events"]=new List<object>();if(JsonUtil.Get(state,"hidden_events")==null)state["hidden_events"]=new List<object>();}
