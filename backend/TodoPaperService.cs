...
internal static partial class TodoApp
{
    ...
    private static void ValidatePaperSettings(PaperSettings settings)
    {
        ...
        if (anyApi && (String.IsNullOrWhiteSpace(settings.ApiBaseUrl) || String.IsNullOrWhiteSpace(settings.Model)))
            throw new Exception("DeepSeek API URL, API Key, and model must be all filled or all empty");
        if (settings.FileServerEnabled && (String.IsNullOrWhiteSpace(settings.FileBaseUrl) || String.IsNullOrWhiteSpace(settings.FileAccount)))
            throw new Exception("File server URL and account cannot be empty when file sync is enabled");
        if (String.IsNullOrWhiteSpace(settings.TitlePrompt) || String.IsNullOrWhiteSpace(settings.AbstractPrompt))
            throw new Exception("Title and abstract scoring prompts cannot be empty");
        if (!settings.TitlePrompt.Contains(PaperListPlaceholder) || !settings.AbstractPrompt.Contains(PaperListPlaceholder))
            throw new Exception("Both title and abstract scoring prompts must contain the paper insertion placeholder " + PaperListPlaceholder);
    }
    ...
    private static void SyncArxiv(Dictionary<string, object> state, bool manual, string paperDate)
    {
        ...
        if (!settings.Enabled)
        {
            if (manual) Meta(state)["status"] = "Paper recommendation disabled";
            return;
        }
        ...
        {
            Meta(state)["status"] = ReadPaperJobMessage("Paper background task is running");
            return;
        }
        ...
        if (TryLoadPapers(finalPath, out local) && IsPaperFileComplete(local, settings))
        {
            ...
            if (remoteSync == "uploaded") Meta(state)["status"] += "; uploaded to file server";
            else if (remoteSync == "exists") Meta(state)["status"] += "; remote file exists, keeping local result";
            else if (remoteSync == "failed") Meta(state)["status"] += "; remote status check failed";
            return;
        }
        ...
        if (HasFileServer(settings))
        {
            ...
            if (remote.Status == "found")
            {
                ...
                remote.Status = "error";
                remote.Error = "Cannot read remote paper JSON";
            }
        }

        if (!manual)
        {
            Meta(state)["status"] = remote == null ? "No local " + today + " scored papers yet" :
                remote.Status == "notfound" ? "No remote " + today + " scored papers yet" : remote.Error;
            return;
        }
        if (JsonUtil.String(Meta(state), "paper_api_skip_date", "") == today)
        {
            Meta(state)["status"] = "DeepSeek skipped for today; will still check remote on refresh";
            return;
        }
        if (!HasScoringApi(settings))
        {
            Meta(state)["status"] = "DeepSeek scoring API is not configured";
            return;
        }

        string prompt;
        if (remote != null && remote.Status == "error")
            prompt = "Cannot confirm remote paper file status, continuing may incur duplicate scoring costs.\r\n\r\nStill fetch arXiv and call DeepSeek scoring locally?";
        else if (remote != null && remote.Status == "notfound")
            prompt = "No remote " + today + " paper file.\r\n\r\nFetch arXiv and call DeepSeek scoring locally?";
        else
            prompt = "No complete local paper cache for " + today + ".\r\n\r\nFetch arXiv and call DeepSeek scoring?";
        ...
        if (consent != "use")
        {
            Meta(state)["status"] = "Local paper scoring cancelled";
            return;
        }
        WritePaperJob("queued", "Background scoring started, preparing papers for " + today + " papers", 0, 0);
        StartPaperWorker(today);
        Meta(state)["status"] = "Background paper scoring started";
    }
    ...
    {
        ValidatePaperSettings(settings);
        if (!settings.Enabled) throw new Exception("Please enable paper recommendation first");
        if (!HasScoringApi(settings)) throw new Exception("Please fill in complete DeepSeek API configuration first");
        if (IsPaperJobRunning()) throw new Exception(ReadPaperJobMessage("Paper background task is running, please wait for it to complete"));
        ...
        })
        {
            ...
            catch (Exception ex) { throw new Exception("Cannot clear today's paper cache: " + ex.Message); }
        }
        File.WriteAllText(PaperRescorePath(date), RuntimeUtil.Iso(DateTimeOffset.Now), RuntimeUtil.Utf8NoBom);
        WritePaperJob("queued", "Preparing to refetch papers for " + date + " papers", 0, 0);
        int reset = WithLockedState(delegate(Dictionary<string, object> state, ref bool refresh) {
            ...
            Meta(state)["status"] = "Rescoring started with current settings";
            Commit(state);
            refresh = true;
        });
        if (reset != 0)
        {
            ...
            WritePaperJob("failed", "Rescoring not started: cannot update todo status", 0, 0);
            throw new Exception("Cannot update todo status, please try again later");
        }
        try { StartPaperWorker(date); }
        catch
        {
            try { File.Delete(PaperRescorePath(date)); } catch { }
            WritePaperJob("failed", "Rescoring not started: cannot start background task", 0, 0);
            throw;
        }
        return true;
    }
    ...
    private static int RunPaperWorker(string date)
    {
        using (Mutex mutex = new Mutex(false, PaperWorkerMutexName))
        {
            ...
            try
            {
                ...
                if (!held) return 0;
                PaperSettings settings = LoadPaperSettings();
                if (!HasScoringApi(settings)) { WritePaperJob("failed", "DeepSeek scoring API is not completely configured", 0, 0); return 2; }
                ...
                {
                    WritePaperJob("fetching", "Fetching from arXiv for " + date + " papers", 0, 0);
                    papers = FetchArxivPapers(settings, date);
                    if (papers.Count == 0)
                    {
                        WritePaperJob("completed", date + " has no new eligible papers", 0, 0);
                        UpdatePaperStatus(date + " has no new eligible papers", false);
                        return 0;
                    }
                    JsonUtil.SaveAtomic(partialPath, papers);
                }
                ScorePapers(papers, settings, partialPath);
                if (!IsPaperFileComplete(papers, settings)) throw new Exception("Paper scoring was not completely finished");
                ...
                WritePaperJob("importing", "Scoring completed, importing to local todos", papers.Count, papers.Count);
                ...
                int result = WithLockedState(delegate(Dictionary<string, object> state, ref bool refresh) {
                    ...
                    try
                    {
                        ...
                        if (uploadStatus == "failed") Meta(state)["status"] += "; file server upload failed";
                        else if (uploadStatus == "skipped") Meta(state)["status"] += "; original remote file kept";
                        Commit(state);
                        refresh = true;
                    }
                    catch
                ...
                string completion = result == 0 ? "Paper scoring and todo sync completed" : "Scoring completed, but todo sync failed";
                if (result == 0 && uploadStatus == "skipped") completion += "; remote file not overwritten";
                else if (result == 0 && uploadStatus == "failed") completion += "; remote upload failed";
                WritePaperJob(result == 0 ? "completed" : "failed", completion, papers.Count, papers.Count);
                return result;
            }
            catch (Exception ex)
            {
                WritePaperJob("failed", "Paper scoring failed: " + SafeStatusMessage(ex.Message), 0, 0);
                UpdatePaperStatus("Paper scoring failed: " + SafeStatusMessage(ex.Message), true);
                return 1;
            }
    ...
    {
        return IsPaperJobRunning() ? ReadPaperJobDesktopMessage("Paper background scoring is running") : JsonUtil.String(Meta(state), "status", "Ready");
    }
    ...
    private static List<Dictionary<string, object>> FetchArxivPapers(PaperSettings settings, string date)
    {
        ...
        XmlDocument document = FetchArxivXml(feedPath, settings.TimeoutSeconds * 1000,
            delegate(long received, long total) {
                ...
                string shortMessage = "Fetching papers for " + shortDate + " papers";
                string desktopMessage = shortMessage;
                if (total > 0) desktopMessage += " - Downloaded " + FormatPaperMegabytes(received) + " / " + FormatPaperMegabytes(total);
    ...
    private static XmlDocument FetchArxivXml(string feedPath, int timeout, Action<long, long> progress)
    {
        ...
        throw new Exception("Cannot connect to arXiv, please check network or DNS and refresh" + (last == null ? "" : ": " + SafeStatusMessage(last.Message)));
    }
    ...
    private static void ScoreStage(List<Dictionary<string, object>> stagePapers, string stage, int batchSize, int minimum, int maximum, PaperSettings settings, string partialPath, List<Dictionary<string, object>> allPapers)
    {
        ...
        int completed = 0;
        WritePaperJob(stage, stage == "title" ? "Performing concurrent title scoring" : "Performing concurrent abstract scoring", 0, stagePapers.Count);
        Parallel.ForEach(batches, new ParallelOptions { MaxDegreeOfParallelism = settings.MaxConcurrency }, delegate(List<Dictionary<string, object>> batch) {
            ...
            lock (saveLock)
            {
                ...
                WritePaperJob(stage, (stage == "title" ? "Title scoring " : "Abstract scoring ") + completed + "/" + stagePapers.Count, completed, stagePapers.Count);
            }
        });
    }
    ...
    private static Dictionary<int, int> ScoreBatchWithRecovery(List<Dictionary<string, object>> batch, string stage, int minimum, int maximum, PaperSettings settings)
    {
        ...
        throw new Exception("Paper " + Convert.ToString(JsonUtil.Get(batch[0], "id"), CultureInfo.InvariantCulture) + " scoring failed: " + (last == null ? "Unknown error" : last.Message));
    }
    ...
    private static Dictionary<int, int> CallDeepSeekBatch(List<Dictionary<string, object>> batch, string stage, int minimum, int maximum, PaperSettings settings)
    {
        ...
        if (choices.Count == 0) throw new Exception("DeepSeek did not return choices");
    ...
    private static Dictionary<int, int> ParseDeepSeekScores(string content, List<Dictionary<string, object>> batch, int minimum, int maximum)
    {
        ...
        foreach (KeyValuePair<string, object> pair in values)
        {
            ...
            if (!Int32.TryParse(pair.Key, NumberStyles.Integer, CultureInfo.InvariantCulture, out id) ||
                ...
                throw new Exception("DeepSeek returned non-integer ID or score");
            if (score < minimum || score > maximum) throw new Exception("DeepSeek returned score out of bounds " + minimum + "-" + maximum);
            result[id] = score;
        }
        if (!expected.SetEquals(result.Keys)) throw new Exception("DeepSeek returned paper IDs inconsistent with request batch");
        return result;
    }
    ...
    {
        ValidatePaperSettings(settings);
        if (!HasFileServer(settings)) throw new Exception("Please enable and fill in complete file server configuration first");
    ...
    private static RemotePaperResult DownloadRemotePaper(PaperSettings settings, string path)
    {
        ...
        {
            return new RemotePaperResult { Status = ex.StatusCode == 404 ? "notfound" : "error", Error = ex.StatusCode == 404 ? "" : "Paper file server connection failed" };
        }
        catch { return new RemotePaperResult { Status = "error", Error = "Paper file server connection failed" }; }
    }
    ...
    private static string UploadScoredPaper(PaperSettings settings, string path, int paperCount)
    {
        ...
        try
        {
            ...
            if (remote.Status == "found")
            {
                WritePaperJob("upload_wait", "Remote has a paper file with the same name, waiting for overwrite confirmation", paperCount, paperCount);
                if (!ShowPaperOverwriteConsent(Path.GetFileName(path))) return "skipped";
                overwrite = true;
            }
            WritePaperJob("uploading", overwrite ? "Overwriting remote paper file" : "Syncing paper to file server", paperCount, paperCount);
    ...
    private static RemotePaperResult CheckRemotePaper(PaperSettings settings, string token, string path)
    {
        ...
        catch (PaperHttpException ex)
        {
            return new RemotePaperResult {
                ...
                Error = ex.StatusCode == 404 ? "" : "Cannot confirm remote paper file status"
            };
        }
        catch { return new RemotePaperResult { Status = "error", Error = "Cannot confirm remote paper file status" }; }
    }
    ...
    private static void ImportPapers(Dictionary<string, object> state, List<Dictionary<string, object>> papers, string date, PaperSettings settings)
    {
        ...
        if (ranked.Count == 0) { Meta(state)["status"] = date + " has no papers passed abstract scoring"; return; }
        ...
        foreach (Dictionary<string, object> paper in ranked)
        {
            ...
            EditorResult editor = new EditorResult {
                ...
                Target = target,
                Note = "Original paper title: " + original + "\r\narXiv ID: " + arxiv,
                Labels = new List<string>{"Paper"}, Available = "", Due = ""
            };
            Tasks(state).Add(NewTask(editor, "arxiv"));
            added++;
        }
        Meta(state)["last_arxiv_sync_date"] = date;
        Meta(state)["status"] = added > 0 ? "Added " + date + " total " + added + " papers, translated " + translated + " papers" : date + " recommended papers already exist";
    }
