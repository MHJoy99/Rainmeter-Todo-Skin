...
internal static partial class TodoApp
{
    ...
    {
        if (!File.Exists(UpdaterScript)) throw new Exception("Standalone updater script not found: " + UpdaterScript);
    ...
    private static UpdateCheckResult CheckLatestUpdate()
    {
        ...
        if (tag == "") throw new Exception("No version tag available on GitHub");
    ...
    private static string CurrentRainmeterRoot()
    {
        ...
        if (root == null || skins == null || !skins.Name.Equals("Skins", StringComparison.OrdinalIgnoreCase)) throw new Exception("Cannot locate the current Rainmeter skins directory");
        if (!Directory.Exists(Path.Combine(root.FullName, "Skins"))) throw new Exception("Cannot locate the current Rainmeter skins directory");
    ...
    private static void SaveTranslationCredentials(string secretId, string secretKey)
    {
        ...
        if (secretId == "" || secretKey == "") throw new Exception("SecretId and SecretKey cannot be empty");
    ...
    private static string TestTranslationCredentials(string secretId, string secretKey)
    {
        ...
        return result == "" ? "Translation service is available" : result;
    }
    ...
    private static string TranslateWithCredentials(Dictionary<string, object> credentials, string text)
    {
        ...
        if (id == "" || key == "") throw new Exception("SecretId and SecretKey cannot be empty");
        ...
        if (translated == "") throw new Exception("Tencent Cloud returned no translation response");
        return translated;
    }
