    ...
    private static void Main(string[] args)
    {
        ...
        try
        {
            ...
            Dictionary<string, object> todo = new Dictionary<string, object> {
                {"version", 1}, {"meta", new Dictionary<string,object>{{"last_arxiv_sync_date",""},{"status","Ready"}}},
                {"tasks", new object[] {
                    new Dictionary<string,object>{{"id","11111111111111111111111111111111"},{"title","Normal task"},{"target",""},{"completed",false},{"source","manual"},{"created_at",now},{"completed_at",null}},
                    new Dictionary<string,object>{{"id","22222222222222222222222222222222"},{"title","Paper"},{"translated_title","Paper"},{"abstract_score",9.5},{"arxiv_id","2601.00001"},{"target","https://arxiv.org/html/2601.00001"},{"completed",false},{"source","arxiv"},{"created_at",now},{"completed_at",null}}
                }}
            };
            ...
            Check(File.ReadAllText(Path.Combine(todoDir,"Generated.inc"),new UTF8Encoding(false)).Contains("TodoHost.exe\" \"Toggle"), "Todo actions were not generated for the C# host");
            ...
            Dictionary<string,object> testEvent=new Dictionary<string,object> {
                ...
                {"title","Test event"},{"start_at",start.ToString("o")},{"end_at",end.ToString("o")},{"all_day",false},
                {"url",""},{"location","Conference room"},{"description",""},{"status",""},{"reminder_at",""},{"reminder_count",0},{"recurring",false}
            };
            Dictionary<string,object> cache=new Dictionary<string,object> {
                {"version",1},{"fetched_at",now},{"calendar_url","https://example.invalid/calendar/"},{"status","Test"},{"events",new object[]{testEvent}}
            };
            ...
            string generated=File.ReadAllText(Path.Combine(calendarDir,"Generated.inc"),new UTF8Encoding(false));
            Check(generated.Contains("Test event") && generated.Contains("CalendarHost.exe\" \"Detail"),"Calendar render/action generation failed");
            ...
            string refreshedTodo=File.ReadAllText(Path.Combine(todoDir,"Generated.inc"),new UTF8Encoding(false));
            ...
            string noCalDav=File.ReadAllText(Path.Combine(calendarDir,"Generated.inc"),new UTF8Encoding(false));
            Check(noCalDav.Contains("CalDAV not connected"),"Calendar without credentials did not settle on disconnected status");
