...
internal static partial class CalendarApp
{
    private static bool EditInteractive(Dictionary<string,object> original,Dictionary<string,object> state,Dictionary<string,object> cache)
    {
        ...
        Form f=LightUi.Form(isNew?"New event":"Edit event",640,800);LightUi.Heading(f,isNew?"New event":"Edit event",isNew?"Create local event or sync to CalDAV calendar.":"Edit event info, delete button is at the bottom.",isNew?"new-calendar.svg":null);
        ...
        f.Controls.Add(new Label{Text="Calendar",Left=26,Top=92,Width=120,Height=22,BackColor=Color.Transparent,ForeColor=LightUi.Text,Font=new System.Drawing.Font("Microsoft YaHei UI",9.5F,System.Drawing.FontStyle.Bold)});
        ...
        Button localSource=LightUi.Button("Local calendar",26,118,110,DialogResult.None),caldavSource=LightUi.Button("CalDAV calendar",144,118,124,DialogResult.None);
        ...
        TextBox title=addField("Title *",26,178,588,38,isNew?"":CleanTitle(S(original,"title")));
        f.Controls.Add(new Label{Text="Date & time",Left=26,Top=240,Width=160,Height=22,BackColor=Color.Transparent,ForeColor=LightUi.Text,Font=new System.Drawing.Font("Microsoft YaHei UI",9.5F,System.Drawing.FontStyle.Bold)});
        ...
        prevDate.Height=dateButton.Height=nextDate.Height=36;dateButton.TextAlign=ContentAlignment.MiddleCenter;Button allDay=LightUi.Button("All day",282,268,82,DialogResult.None);allDay.Height=36;allDay.TextAlign=ContentAlignment.MiddleCenter;f.Controls.AddRange(new Control[]{prevDate,dateButton,nextDate,allDay});
        Panel timeGroup=new Panel{Left=26,Top=314,Width=588,Height=66,BackColor=Color.Transparent};
        Label startLabel=new Label{Text="Start time  "+selectedStart.ToString(@"hh\:mm"),Left=0,Top=0,Width=240,Height=20,BackColor=Color.Transparent,ForeColor=LightUi.Muted,Font=new System.Drawing.Font("Microsoft YaHei UI",9F)};
        Label endLabel=new Label{Text="End time  "+selectedEnd.ToString(@"hh\:mm"),Left=294,Top=0,Width=240,Height=20,BackColor=Color.Transparent,ForeColor=LightUi.Muted,Font=new System.Drawing.Font("Microsoft YaHei UI",9F)};
        ...
        Action updateTimeLabels=delegate{selectedStart=TimeSpan.FromMinutes(startSlider.Value*15);selectedEnd=TimeSpan.FromMinutes(endSlider.Value*15);startLabel.Text="Start time  "+selectedStart.ToString(@"hh\:mm");endLabel.Text="End time  "+selectedEnd.ToString(@"hh\:mm");};
        ...
        TextBox location=addField("Location",26,548,588,38,isNew?"":S(original,"location"));location.Text=location.Text==""?"Add location":location.Text;location.ForeColor=S(original,"location")==""?Color.FromArgb(150,165,185):LightUi.Text;location.GotFocus+=delegate{if(location.Text=="Add location"){location.Text="";location.ForeColor=LightUi.Text;}};location.LostFocus+=delegate{if(location.Text.Trim()==""){location.Text="Add location";location.ForeColor=Color.FromArgb(150,165,185);}};
        TextBox url=addField("Link",26,612,588,38,isNew?"":S(original,"url"));url.Text=url.Text==""?"Add meeting link, website or local path":url.Text;url.ForeColor=S(original,"url")==""?Color.FromArgb(150,165,185):LightUi.Text;url.GotFocus+=delegate{if(url.Text=="Add meeting link, website or local path"){url.Text="";url.ForeColor=LightUi.Text;}};url.LostFocus+=delegate{if(url.Text.Trim()==""){url.Text="Add meeting link, website or local path";url.ForeColor=Color.FromArgb(150,165,185);}};
        f.Controls.Add(new Label{Text="Reminder",Left=26,Top=404,Width=120,Height=22,BackColor=Color.Transparent,ForeColor=LightUi.Text,Font=new System.Drawing.Font("Microsoft YaHei UI",9.5F,System.Drawing.FontStyle.Bold)});
        ...
        Label reminderText=new Label{Text="Remind me before start",Left=50,Top=16,Width=172,Height=24,BackColor=Color.Transparent,ForeColor=LightUi.Text,Font=new System.Drawing.Font("Microsoft YaHei UI",10F)};
        Button addReminder=LightUi.Button("+ Add reminder",392,10,98,DialogResult.None);addReminder.Height=34;addReminder.TextAlign=ContentAlignment.MiddleCenter;addReminder.ForeColor=LightUi.Accent;addReminder.BackColor=Color.FromArgb(235,245,253);addReminder.UseVisualStyleBackColor=false;
        Button expandReminder=LightUi.Button("Expand",500,10,66,DialogResult.None);expandReminder.Height=34;expandReminder.TextAlign=ContentAlignment.MiddleCenter;expandReminder.ForeColor=LightUi.Muted;expandReminder.BackColor=Color.FromArgb(235,245,253);
        ...
        Func<int,string> reminderLabel=delegate(int m){if(m>=1440&&m%1440==0)return (m/1440)+" days";if(m>=60&&m%60==0)return (m/60)+" hours";return m+" mins";};
        Action renderReminders=null;
        Action showAddReminderDialog=delegate{Form rf=LightUi.Form("Add reminder",360,210);LightUi.Heading(rf,"Add reminder","Only reminders before event start are allowed.");TextBox num=new TextBox{Left=36,Top=94,Width=110,Height=30,Text="15",Font=new System.Drawing.Font("Microsoft YaHei UI",11F)};ComboBox unit=new ComboBox{Left=158,Top=92,Width=100,Height=34,DropDownStyle=ComboBoxStyle.DropDownList,Font=new System.Drawing.Font("Microsoft YaHei UI",10F)};unit.Items.AddRange(new object[]{"Minutes","Hours","Days"});unit.SelectedIndex=0;Button ok=LightUi.PrimaryButton("Add",204,150,72,DialogResult.OK);Button dialogCancel=LightUi.Button("Cancel",282,150,56,DialogResult.Cancel);rf.Controls.AddRange(new Control[]{num,unit,ok,dialogCancel});rf.AcceptButton=ok;rf.CancelButton=dialogCancel;if(rf.ShowDialog()==DialogResult.OK){int value;if(Int32.TryParse(num.Text.Trim(),out value)&&value>0){int minutes=value*(unit.SelectedIndex==2?1440:unit.SelectedIndex==1?60:1);if(!reminderMinutes.Contains(minutes))reminderMinutes.Add(minutes);renderReminders();}else LightUi.Error("Please enter a valid number");}};
        ...
        renderReminders();
        TextBox description=addField("Notes",26,674,588,42,isNew?"":S(original,"description"));description.Multiline=true;description.ScrollBars=ScrollBars.None;description.Text=description.Text==""?"Add notes":description.Text;description.ForeColor=S(original,"description")==""?Color.FromArgb(150,165,185):LightUi.Text;description.GotFocus+=delegate{if(description.Text=="Add notes"){description.Text="";description.ForeColor=LightUi.Text;}};description.LostFocus+=delegate{if(description.Text.Trim()==""){description.Text="Add notes";description.ForeColor=Color.FromArgb(150,165,185);}};
        ...
        Label hint=LightUi.Label(recurringCalDav?"Saving will overwrite the entire CalDAV recurring event series.":hasCalDav?"CalDAV configured, can sync to remote calendar.":"Creates a local calendar event when CalDAV is not configured.",128,17,270);footer.Controls.Add(hint);
        Button delete=LightUi.DangerButton("Delete event",18,5,96,DialogResult.None);delete.Visible=!isNew;Button cancel=LightUi.Button("Cancel",416,5,74,DialogResult.Cancel);Button save=LightUi.PrimaryButton("Save",500,5,74,DialogResult.None);footer.Controls.AddRange(new Control[]{delete,cancel,save});f.Controls.Add(footer);f.CancelButton=cancel;
        delete.BringToFront();cancel.BringToFront();save.BringToFront();
        bool deleted=false;
        bool saved=false;
        Func<bool> saveDraft=delegate{
            string cleanTitle=title.Text.Trim();if(cleanTitle==""){LightUi.Error("Title cannot be empty");return false;}
            ...
            DateTimeOffset end=allDaySelected?start.AddDays(1):new DateTimeOffset(day.Year,day.Month,day.Day,selectedEnd.Hours,selectedEnd.Minutes,0,TimeZoneInfo.Local.GetUtcOffset(day));if(end<=start){LightUi.Error("End time cannot be earlier than start time");return false;}
            if(recurringCalDav){DialogResult confirm=MessageBox.Show("This will modify the entire recurring series. All subsequent occurrences will be updated. Are you sure you want to continue?","Rewrite recurring event",MessageBoxButtons.YesNo,MessageBoxIcon.Warning);if(confirm!=DialogResult.Yes)return false;}
            if(originalCustomAlarmCount>customAlarms.Count){DialogResult confirmCustom=MessageBox.Show("You deleted extra CalDAV reminders. Saving will remove them from the remote event, and you cannot recreate them here later. Are you sure you want to continue?","Delete extra CalDAV reminders",MessageBoxButtons.YesNo,MessageBoxIcon.Warning);if(confirmCustom!=DialogResult.Yes)return false;}
            string locationText=location.Text.Trim()=="Add location"?"":location.Text.Trim(),urlText=url.Text.Trim()=="Add meeting link, website or local path"?"":url.Text.Trim(),descriptionText=description.Text.Trim()=="Add notes"?"":description.Text.Trim();
            ...
            try{
                if(selected=="caldav"){
                    ...
                    SaveLocalEvent(draft,state);cache["status"]="Saved to local calendar";
                }
                return true;
            }catch(Exception ex){
                if(selected=="caldav"&&isNew){
                    ...
                    selectedSource="local";caldavSource.Enabled=false;hint.Text="CalDAV connection failed, saved to local calendar.";paintSource();
                    cache["status"]="CalDAV save failed, saved locally: "+ex.Message;
                    LightUi.Error("CalDAV connection timed out or failed. Saved as local event. You can edit and switch it to CalDAV later.");
                    return true;
                }
                LightUi.Error(ex.Message);return false;
            }
        };
        delete.Click+=delegate{string mode="series";if(B(original,"recurring")){DialogResult choice=MessageBox.Show("This is a recurring event. Select 'Yes' to delete the entire series, or 'No' to hide only this instance locally.","Delete recurring event",MessageBoxButtons.YesNoCancel,MessageBoxIcon.Warning);if(choice==DialogResult.Cancel)return;mode=choice==DialogResult.Yes?"series":"once";}else if(!LightUi.Confirm("Are you sure you want to delete this event?","Delete event"))return;try{if(originalCalDav)DeleteCalDavEvent(original,cache,state,mode);else DeleteLocalEvent(original,state);deleted=true;f.DialogResult=DialogResult.OK;f.Close();}catch(Exception ex){LightUi.Error(ex.Message);}};
    ...
    private static void SaveCredentials(TextBox server, TextBox username, TextBox password, Dictionary<string,object> cache)
    {
        ...
        cache["fetched_at"] = "";
        cache["status"] = "CalDAV credentials saved";
    ...
    private static string TestCredentials(TextBox server, TextBox username, TextBox password)
    {
        ...
        return "Connection successful: " + (calendar.Name == "" ? calendar.Uri : calendar.Name);
    }
    ...
    private static void ClearCredentials(Dictionary<string,object> cache)
    {
        ...
        cache["calendar_url"] = "";
        cache["fetched_at"] = "";
        cache["status"] = "CalDAV not connected";
        Save(CachePath, cache);
    }
    ...
    {
        list.Items.Clear();
        foreach (Dictionary<string,object> rule in Rules(state)) list.Items.Add(new KeyValuePair<Dictionary<string,object>,string>(rule, S(rule, "title") == "" ? "Recurring event" : S(rule, "title")));
        if (list.Items.Count == 0) list.Items.Add(new KeyValuePair<Dictionary<string,object>,string>(null, "No auto-import rules yet"));
    }
    ...
    private static void Settings(Dictionary<string,object> state, Dictionary<string,object> cache, ref bool refreshTodo)
    {
        ...
        Form f = LightUi.Form("Event Settings", 720, 560);
        ...
        Label title = new Label { Text = "Event Settings", Left = 112, Top = 34, Width = 240, Height = 40, BackColor = Color.Transparent, ForeColor = LightUi.Text, Font = new System.Drawing.Font("Microsoft YaHei UI", 18F, System.Drawing.FontStyle.Bold) };
        Label subtitle = new Label { Text = "Manage CalDAV sync account and recurring auto-import rules.", Left = 42, Top = 92, Width = 540, Height = 26, BackColor = Color.Transparent, ForeColor = Color.FromArgb(76, 94, 132), Font = new System.Drawing.Font("Microsoft YaHei UI", 10F) };
        ...
        Button tabAccount = LightUi.Button("Sync Account", 0, 0, 136, DialogResult.None);
        Button tabRules = LightUi.Button("Auto-import", 136, 0, 136, DialogResult.None);
        ...
        Panel reveal;
        TextBox server = AddCredentialField(accountPage, "globe", "CalDAV URL", 10, S(credentials, "Server"), false, out reveal);
        TextBox username = AddCredentialField(accountPage, "user", "Username", 92, S(credentials, "Username"), false, out reveal);
        TextBox password = AddCredentialField(accountPage, "lock", "Password", 174, S(credentials, "Password"), true, out reveal);
        ...
        rulePage.Controls.Add(new Label { Text = "Stopping a rule only affects future recurrences, already generated tasks remain unchanged.", Left = 34, Top = 232, Width = 420, Height = 24, ForeColor = Color.FromArgb(76, 94, 132), BackColor = Color.Transparent, Font = new System.Drawing.Font("Microsoft YaHei UI", 9F) });
        Button stop = LightUi.DangerButton("Clear rule", 506, 224, 116, DialogResult.None);
        stop.Height = 40;
        stop.Font = new System.Drawing.Font("Microsoft YaHei UI", 9F, System.Drawing.FontStyle.Bold);
        rulePage.Controls.Add(stop);

        Label saveStatus = new Label { Text = S(cache, "status"), Left = 64, Top = 502, Width = 280, Height = 28, BackColor = Color.Transparent, ForeColor = S(cache, "status").Contains("successful") || S(cache, "status").Contains("synced") || S(cache, "status").Contains("saved") ? Color.FromArgb(63, 178, 119) : Color.FromArgb(76, 94, 132), Font = new System.Drawing.Font("Microsoft YaHei UI", 10F, System.Drawing.FontStyle.Bold) };
        Button clearAccount = LightUi.DangerButton("Clear settings", 364, 494, 98, DialogResult.None);
        Button testAccount = LightUi.Button("Test connection", 476, 494, 106, DialogResult.None);
        Button saveAccount = LightUi.PrimaryButton("Save credentials", 590, 494, 110, DialogResult.None);
        ...
        testAccount.Click += delegate {
            try { saveStatus.Text = "Testing..."; saveStatus.ForeColor = Color.FromArgb(76, 94, 132); saveStatus.Refresh(); saveStatus.Text = TestCredentials(server, username, password); saveStatus.ForeColor = Color.FromArgb(63, 178, 119); }
            catch (Exception ex) { LightUi.Error("Connection failed: " + ex.Message); saveStatus.Text = "Connection failed"; saveStatus.ForeColor = LightUi.Danger; }
        };
        saveAccount.Click += delegate {
            try { SaveCredentials(server, username, password, cache); saveStatus.Text = "Saved"; saveStatus.ForeColor = Color.FromArgb(63, 178, 119); }
            catch (Exception ex) { LightUi.Error(ex.Message); }
        };
        clearAccount.Click += delegate {
            try { ClearCredentials(cache); server.Text = ""; username.Text = ""; password.Text = ""; saveStatus.Text = "CalDAV not connected"; saveStatus.ForeColor = Color.FromArgb(76, 94, 132); }
            catch (Exception ex) { LightUi.Error(ex.Message); }
        };
        stop.Click += delegate {
            ...
            cache["status"] = "Stopped future auto-import for this series";
            Save(CachePath, cache);
            FillRules(list, state);
            saveStatus.Text = "Stopped future auto-import for this series";
    ...
    private static bool ManageEvents(Dictionary<string,object> state, Dictionary<string,object> cache, ref bool refreshTodo)
    {
        ...
        Form f = LightUi.Form("Manage Events", 1180, 760);
        ...
        Label title = new Label { Text = "Manage Events", Left = 90, Top = 24, Width = 260, Height = 38, BackColor = Color.Transparent, ForeColor = LightUi.Text, Font = new System.Drawing.Font("Microsoft YaHei UI", 18F, System.Drawing.FontStyle.Bold) };
        Label subtitle = new Label { Text = "View, edit and sync your local and CalDAV calendars.", Left = 92, Top = 70, Width = 520, Height = 22, BackColor = Color.Transparent, ForeColor = LightUi.Muted, Font = new System.Drawing.Font("Microsoft YaHei UI", 9.5F) };
        ...
        Label monthTitle = new Label { Left = 70, Top = 29, Width = 220, Height = 24, BackColor = Color.Transparent, ForeColor = LightUi.Text, TextAlign = ContentAlignment.MiddleCenter, Font = new System.Drawing.Font("Microsoft YaHei UI", 12F, System.Drawing.FontStyle.Bold) };
        Button todayButton = LightUi.Button("Today", 238, 66, 70, DialogResult.None); todayButton.Height = 32;
        ...
        Label filterTitle = new Label { Text = "Filter calendars", Left = 18, Top = 14, Width = 160, Height = 24, BackColor = Color.Transparent, ForeColor = LightUi.Text, Font = new System.Drawing.Font("Microsoft YaHei UI", 10.5F, System.Drawing.FontStyle.Bold) };
        Button localFilter = LightUi.Button("✓  Local", 18, 48, 120, DialogResult.None);
        Button caldavFilter = LightUi.Button("✓  CalDAV", 150, 48, 120, DialogResult.None);
        ...
        int timeMode = 0;
        Panel timeTabs = RoundedPanel(22, 22, 254, 38, Color.FromArgb(235,245,253), Color.FromArgb(235,245,253), 10);
        Button todayTab = LightUi.Button("Today", 2, 2, 62, DialogResult.None);
        Button weekTab = LightUi.Button("Next 7 days", 66, 2, 96, DialogResult.None);
        Button allTimeTab = LightUi.Button("All time", 164, 2, 88, DialogResult.None);
        ...
        Label footerSummary = LightUi.Label("Current date: " + selectedDate.ToString("yyyy/M/d") + " · 0 events", 22, 18, 460); footer.Controls.Add(footerSummary);
        Button settings = LightUi.Button("Settings", 758, 10, 92, DialogResult.None);
        Button sync = LightUi.Button("Refresh sync", 858, 10, 112, DialogResult.None);
        Button add = LightUi.PrimaryButton("New event", 978, 10, 112, DialogResult.None);
        ...
        Action updateFilters = delegate {
            ...
            localFilter.Text=(showLocal?"✓  ":"")+"Local  "+localCount;
        ...
        renderCalendar = delegate {
            ...
            monthTitle.Text=selectedDate.ToString("yyyy-MM",CultureInfo.InvariantCulture);
        ...
        reload = delegate {
            ...
            if(timeMode==2){dayHeader.Text="All time · "+rows.Count+" events";footerSummary.Text="All time · "+rows.Count+" events";}
            else if(timeMode==1){DateTime rangeEnd=selectedDate.AddDays(6);dayHeader.Text=selectedDate.ToString("yyyy-MM-dd",CultureInfo.InvariantCulture)+" - "+rangeEnd.ToString("MM-dd",CultureInfo.InvariantCulture)+" · "+rows.Count+" events";footerSummary.Text="Next 7 days: "+selectedDate.ToString("yyyy/M/d")+" - "+rangeEnd.ToString("yyyy/M/d")+" · "+rows.Count+" events";}
            else{dayHeader.Text=selectedDate.ToString("yyyy-MM-dd dddd",CultureInfo.InvariantCulture)+" · "+rows.Count+" events";footerSummary.Text="Current date: "+selectedDate.ToString("yyyy/M/d")+" · "+rows.Count+" events";}
            ...
            foreach(Dictionary<string,object> e in rows)
            {
                ...
                string timeText=B(e,"all_day")?"All day":es.ToString("HH:mm")+" - "+ee.ToString("HH:mm");
                if(showEventDate)timeText=es.ToString("M/d ddd",CultureInfo.InvariantCulture)+"\r\n"+timeText;
                ...
                Label loc=new Label{Text=noLocation?"No location":S(e,"location"),Left=172,Top=43,Width=310,Height=18,BackColor=Color.Transparent,ForeColor=noLocation?Color.FromArgb(165,181,202):LightUi.Muted,Font=new System.Drawing.Font("Microsoft YaHei UI",noLocation?8F:9F)};
                Label badge=new Label{Text=caldav?"CalDAV calendar":"Local calendar",Left=172,Top=60,Width=118,Height=22,BackColor=caldav?Color.FromArgb(221,237,255):Color.FromArgb(218,246,231),ForeColor=caldav?LightUi.Accent:Color.FromArgb(35,145,89),TextAlign=ContentAlignment.MiddleCenter,Font=new System.Drawing.Font("Microsoft YaHei UI",9F)};
            ...
            if(rows.Count==0){Panel empty=RoundedPanel(0,0,694,86,Color.FromArgb(252,254,255),Color.FromArgb(224,233,244),14);empty.Controls.Add(new Label{Text="No events on this day",Left=230,Top=30,Width=240,Height=24,TextAlign=ContentAlignment.MiddleCenter,BackColor=Color.Transparent,ForeColor=LightUi.Muted,Font=new System.Drawing.Font("Microsoft YaHei UI",10F)});list.Controls.Add(empty);}
        };
        ...
        sync.Click += delegate {
            ...
            if(syncProcess==null){sync.Text="Refresh sync";return;}
            sync.Enabled=false;sync.Text="Syncing...";
            cache["status"]="Syncing in background";Save(CachePath,cache);Render(cache,state);reload();
            ...
            syncTimer.Tick+=delegate{
                ...
                sync.Enabled=true;sync.Text="Refresh sync";reload();
            };
            syncTimer.Start();
        };
    ...
    private static void Manage(Dictionary<string,object> state, Dictionary<string,object> cache)
    {
        while (true)
        {
            Form f = LightUi.Form("Recurring auto-import rules", 620, 500);
            LightUi.Heading(f, "Recurring auto-import rules", "Stopping a rule only affects future recurrences, already generated tasks remain unchanged.");
            ...
            foreach (Dictionary<string,object> rule in Rules(state)) list.Items.Add(new KeyValuePair<Dictionary<string,object>,string>(rule, S(rule, "title") == "" ? "Recurring event" : S(rule, "title")));
            if (list.Items.Count == 0) list.Items.Add(new KeyValuePair<Dictionary<string,object>,string>(null, "No auto-import rules yet"));
            f.Controls.Add(list); f.Controls.Add(LightUi.Label("Select a rule to stop future auto-imports.", 25, 404, 360));
            Button stop = LightUi.DangerButton("Stop auto-import", 374, 430, 130, DialogResult.OK), close = LightUi.Button("Close", 514, 430, 82, DialogResult.Cancel);
            ...
            Rules(state).Remove(selected); Save(StatePath, state); cache["status"] = "Stopped future auto-import for this series"; Save(CachePath, cache);
        }
    }
