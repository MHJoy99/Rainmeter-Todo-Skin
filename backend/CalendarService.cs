...
internal static partial class CalendarApp
{
    private static void RefreshCalDavCache(Dictionary<string,object> cache,Dictionary<string,object> credentials,string status)
    {
        ...
        cache["status"]=status+(r.FailedWindows>0?" ("+r.FailedWindows+" query timeout)":"");
    }
    private static void Sync(Dictionary<string,object>cache,Dictionary<string,object>state,ref bool todo){try{if(!File.Exists(SecretPath)){cache["events"]=new List<object>();cache["calendar_url"]="";cache["fetched_at"]="";cache["status"]="CalDAV not connected";Save(CachePath,cache);return;}Dictionary<string,object>c=JsonUtil.ReadDpapiJson(SecretPath);FetchResult r=Fetch(c,S(cache,"calendar_url"));cache["events"]=r.Events.Cast<object>().ToList();cache["calendar_url"]=r.Calendar.Uri;cache["fetched_at"]=RuntimeUtil.Iso(DateTimeOffset.Now);cache["status"]="Synced "+r.Events.Count+" item(s)"+(r.FailedWindows>0?" ("+r.FailedWindows+" query timeout)":"");if(AutoConvert(cache,state))todo=true;Save(CachePath,cache);Save(StatePath,state);}catch(Exception ex){cache["status"]="Sync failed: "+ex.Message;Save(CachePath,cache);}}
    ...
    {
        Dictionary<string,object> credentials=ReadCredentials();if(credentials.Count==0)throw new Exception("CalDAV not connected. Please save account in Calendar Settings first.");
        ...
        if(put.Status<200||put.Status>=300)throw new Exception("CalDAV save failed: HTTP "+put.Status);
        try{RefreshCalDavCache(cache,credentials,"Saved and synced CalDAV");}
        catch{Events(cache).RemoveAll(x=>S(x,"id")==S(e,"id")||S(x,"uid")==S(e,"uid"));Events(cache).Add(e);cache["status"]="Saved to CalDAV, local cache will align on next sync";}
    }
