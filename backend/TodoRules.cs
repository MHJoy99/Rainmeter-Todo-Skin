...
internal static partial class TodoApp
{
    private static int Normalize(Dictionary<string, object> state)
    {
        ...
        foreach (Dictionary<string, object> task in tasks.ToList())
        {
            if (S(task, "source") == "arxiv" && (task.ContainsKey("translated_title") || task.ContainsKey("abstract_score") || task.ContainsKey("arxiv_id")))
            {
                ...
                string metadata = "Original paper title: " + original + (arxiv == "" ? "" : "\r\narXiv ID: " + arxiv), note = S(task, "note");
            ...
            if (S(task, "source") == "arxiv") AddLabel(task, "Paper");
            if (S(task, "source") == "caldav") AddLabel(task, "Schedule");
            if (S(task, "source") == "arxiv" && B(task, "completed") && !Labels(task).Contains("Read") && !Labels(task).Contains("Auto-archived"))
            {
                DateTimeOffset? completed = RuntimeUtil.Date(task, "completed_at");
                AddLabel(task, completed.HasValue && completed.Value.Hour == 5 && completed.Value.Minute == 59 ? "Auto-archived" : "Read"); changed++;
            }
            ...
            if (!B(task, "completed") && S(task, "source") == "arxiv")
            {
                ...
                if (created.HasValue && created.Value < window)
                {
                    ...
                    AddLabel(task, "Paper"); AddLabel(task, "Auto-archived"); RemoveLabel(task, "Read"); rolled++;
                }
            }
        }
    ...
    private static void Edit(Dictionary<string, object> state, string id, ref bool refresh)
    {
        ...
        Meta(state)["status"] = "Task updated"; Commit(state); refresh = true;
    }
    private static void Delete(Dictionary<string, object> state, string id, ref bool refresh)
    {
        Dictionary<string, object> task = Find(state, id); if (task == null || !LightUi.Confirm("Delete \"" + S(task, "title") + "\"?", "Delete task")) return;
        Tasks(state).RemoveAll(t => S(t, "id") == id); Meta(state)["status"] = "Deleted"; Commit(state); refresh = true;
    }
    private static void Toggle(Dictionary<string, object> state, string id, ref bool refresh)
    {
        ...
        if (B(task, "completed")) { task["completed"] = false; task["completed_at"] = null; if (S(task, "source") == "arxiv") { task["created_at"] = RuntimeUtil.Iso(DateTimeOffset.Now); RemoveLabel(task, "Read"); RemoveLabel(task, "Auto-archived"); } Meta(state)["status"] = "Restored to pending"; }
        else { task["completed"] = true; task["completed_at"] = RuntimeUtil.Iso(DateTimeOffset.Now); if (Labels(task).Contains("Paper")) { AddLabel(task, "Read"); RemoveLabel(task, "Auto-archived"); } Meta(state)["status"] = "Completed"; }
        Commit(state); refresh = true;
    }
    private static void Open(Dictionary<string, object> state, string id, ref bool refresh)
    {
        Dictionary<string, object> task = Find(state, id); if (task == null) return;
        string rawTarget = S(task, "target").Trim().Trim('"');
        string target = Regex.IsMatch(rawTarget, @"^%[^%]+%") ? Environment.ExpandEnvironmentVariables(rawTarget) : rawTarget;
        if (target.StartsWith("http://") || target.StartsWith("https://") || target.StartsWith("wemeet://") || File.Exists(target) || Directory.Exists(target))
        {
            RuntimeUtil.Run(target);
        }
        else if (target != "")
        {
            Meta(state)["status"] = "Not found: " + Path.GetFileName(target);
            Commit(state); refresh = true;
        }
        else
        {
            string statusMsg;
            bool ok = TodoApp.TryCreateTask(task, out statusMsg);
            Meta(state)["status"] = statusMsg;
            Commit(state); refresh = true;
        }
    }
