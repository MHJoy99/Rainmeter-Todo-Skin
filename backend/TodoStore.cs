...
internal static partial class TodoApp
{
    private static Dictionary<string, object> NewState()
    {
        return new Dictionary<string, object> {
            {"version", 2}, {"meta", new Dictionary<string, object>{{"last_arxiv_sync_date", ""}, {"status", "Ready"}}},
    ...
    private static Dictionary<string, object> LoadState()
    {
        ...
        try
        {
            ...
            state["version"] = 2;
            if (!(JsonUtil.Get(state, "meta") is Dictionary<string, object>)) state["meta"] = new Dictionary<string, object>{{"last_arxiv_sync_date", ""}, {"status", "Ready"}};
            if (JsonUtil.Get(state, "tasks") == null) state["tasks"] = new List<object>();
            return state;
        }
        catch
        {
            try { File.Copy(StatePath, StatePath + ".corrupt-" + DateTime.Now.ToString("yyyyMMdd-HHmmss"), true); } catch { }
            Dictionary<string, object> state = NewState(); Meta(state)["status"] = "Data was corrupted; backed up and rebuilt"; return state;
        }
    }
