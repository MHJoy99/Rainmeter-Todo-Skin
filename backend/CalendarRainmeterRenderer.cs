...
internal static partial class CalendarApp
{
    ...
    private static bool Render(Dictionary<string,object> cache, Dictionary<string,object> state)
    {
        ...
        Meter(lines, "CalendarSummary", "Meter=String", "MeterStyle=StyleText", "X=23", "Y=49", "W=340", "H=16", "FontSize=9", "FontColor=#MutedColor#", "Text=" + now.ToString("MMM d dddd", CultureInfo.InvariantCulture) + "  ·  " + events.Count + " event(s)");
        ...
        if (events.Count == 0)
        {
            Meter(lines, "CalendarEmpty", "Meter=String", "MeterStyle=StyleText", "X=32", "Y=" + (y + 17), "W=456", "H=22", "FontColor=#MutedColor#", "Text=No events scheduled today");
            y += eventHeight;
        }
        else
        {
    ...
    {
        Meter(output, "Header", "Meter=String", "MeterStyle=StyleText", "X=22", "Y=18", "W=330", "H=32", "FontSize=18", "FontWeight=600", "Text=Schedule");
        ...
        Meter(output, "Manage", "Meter=String", "X=441", "Y=40", "W=36", "H=36", "FontFace=Segoe Fluent Icons", "FontSize=12", "FontColor=#MutedColor#", "StringAlign=CenterCenter", "AntiAlias=1", "Text=\xE700", "ToolTipText=Calendar settings", "MouseOverAction=[!SetOption Manage FontColor \"#TextColor#\"][!UpdateMeter Manage][!Redraw]", "MouseLeaveAction=[!SetOption Manage FontColor \"#MutedColor#\"][!UpdateMeter Manage][!Redraw]", "LeftMouseUpAction=[\"#@#CalendarHost.exe\" \"Manage\"]");
        Meter(output, "Sync", "Meter=String", "X=482", "Y=40", "W=36", "H=36", "FontFace=Segoe Fluent Icons", "FontSize=12", "FontColor=225,242,255,255", "StringAlign=CenterCenter", "AntiAlias=1", "Text=\xE72C", "ToolTipText=Sync CalDAV now", "MouseOverAction=[!SetOption Sync FontColor \"255,255,255,255\"][!UpdateMeter Sync][!Redraw]", "MouseLeaveAction=[!SetOption Sync FontColor \"225,242,255,255\"][!UpdateMeter Sync][!Redraw]", "LeftMouseUpAction=[!SetOption Sync FontColor \"#AccentColor#\"][!UpdateMeter Sync][!Redraw][\"#@#CalendarHost.exe\" \"Sync\"]");
    }
    private static string TimeLabel(Dictionary<string,object>e,DateTimeOffset day){DateTimeOffset s=RuntimeUtil.Date(e,"start_at").Value,x=RuntimeUtil.Date(e,"end_at").Value,next=day.AddDays(1);if(B(e,"all_day"))return s.Date<day.Date||x.Date>next.Date?"All day · Cont.":"All day";if(s<day&&x>next)return"All day · Cont.";if(s<day)return"Cont.–"+x.ToString("HH:mm");if(x>next)return s.ToString("HH:mm")+"–Next day";return x<=s?s.ToString("HH:mm"):s.ToString("HH:mm")+"–"+x.ToString("HH:mm");}
